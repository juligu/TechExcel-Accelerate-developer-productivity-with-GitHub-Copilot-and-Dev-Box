...
namespace RazorPagesTestSample.Pages
{
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAnalyzeMessagesAsync()
        {
            ...
            {
                // Remove the Thread.Sleep loop for performance improvement
                var wordCount = 0;
        
                // Use parallel processing to speed up the word counting
                wordCount = Messages.AsParallel().Sum(message => message.Text.Split(' ').Length);
